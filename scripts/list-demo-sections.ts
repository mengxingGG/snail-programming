// 生成「演示课改造成练习课」的工作清单
//
// 判定方式：把每一节的 starterCode 原样跑一遍，再走真实的判题逻辑。
//   demo   → 起始代码不用改就满足输出要求，学生点一下运行就算学完，是改造候选
...
//
// 注意：会真实启动 node / python 子进程，全量跑完需要几分钟。
...
async function main() {
  console.error('开始扫描（会真实执行每一节的起始代码，需要几分钟）...');
  ...
  const total = all.filter(r => r.status === 'demo').length;
